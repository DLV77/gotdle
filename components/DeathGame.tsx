...
import type { DeathClue } from "@/data/deaths";

export default function DeathGame() {
  const { difficulty, config } = useDifficulty();
  const MAX_GUESSES = config.maxGuesses.death;

  function newGame() {
    const pool = getPool(CHARACTERS, config.pool);
    const available = DEATH_CLUES.filter((d) => pool.some((c) => c.id === d.characterId));
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md mx-auto px-4 py-6">
      <div className="text-center">
        ...
        <p className="text-xs text-[#9a8f80] mt-1 uppercase tracking-wider">Death Mode · Who Died This Way?</p>
        <hr className="divider-gold mt-3" />
      </div>

      {/* Death description card */}
      <div className="rounded-2xl border border-[#7f1d1d]/60 bg-[#1a0808] px-5 py-6 relative overflow-hidden">
        <div className="text-6xl absolute -top-2 -right-2 opacity-10">💀</div>
        <p className="text-xs text-[#9a8f80] uppercase tracking-widest mb-2">How did this character die?</p>
        <p className="text-[#e8e0d0] text-base leading-relaxed" style={{ fontFamily: "var(--font-crimson)" }}>
          {clue.clue}
        </p>
        <p className="text-xs text-[#9a8f80] mt-3">Season {clue.season}</p>
      </div>
      ...
      <p className="text-center text-xs text-[#9a8f80]">
        {!gameOver && <>{guesses.length > 0 ? `Guess ${guesses.length} of ${MAX_GUESSES}` : `${MAX_GUESSES} guesses · ${config.emoji} ${config.label}`}</>}
      </p>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {guesses.map((c) => (
            <div key={c.id} className={`flex items-center gap-3 rounded-lg border px-3 py-2 ${c.id === answer.id ? "border-[#166534] bg-[#14532d]" : "border-[#374151] bg-[#1f2937]"}`}>
              ...
              <span className="ml-auto">{c.id === answer.id ? "✓" : "✗"}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {gameOver && (
        <VictoryModal won={won} answer={answer} guessCount={guesses.length} maxGuesses={MAX_GUESSES} onPlayAgain={handlePlayAgain} mode="death" />
      )}
    </div>
  );
}
